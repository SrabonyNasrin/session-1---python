...
def compound_interest(principal_amount, annual_interest_rate, years, compound_per_year):
    
    factor = 1 + annual_interest_rate / compound_per_year
    exponent = compound_per_year * years
    amount = principal_amount * (factor ** exponent)
    return amount
...
compound_per_year = 4  # Number of times interest is compounded per year
result = compound_interest(principal_amount, annual_interest_rate, years, compound_per_year)
print("The future value of the investment is:", result)
...
while True:
    ...
    print("Current datetime:", current_datetime.strftime("%Y-%m-%d %H:%M:%S"), end="\r")  # Print current datetime and overwrite previous output
